...
class PlacesMap extends Component {
    ...
    render() {
        let queryTag = <Query query={PLAC_TAG_Q} variables={{ Tag: this.state.tag_selected }} pollInterval={1000}>
            {({ loading, error, data }) => {
        ...
        let queryCiv = <Query query={PLAC_CIV_Q} variables={{ Id: this.state.idCiv_selected }} pollInterval={1000}>
            {({ loading, error, data }) => {
                if (loading) return <div>Loading</div>
                if (error) return <div>Error</div>
                if (data) {
                            ...
                            {({ loading, error, data }) => {
                                ...
                                if (data) {
                                    ...
                                    var civResult = civList.map(civ =>
                                        <Dropdown.Item key={civ.Id} onClick={() => {
