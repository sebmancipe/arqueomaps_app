...
class MapView extends Component {
  ...
  updateEdges(edges) {
    ...
    const firstPlace = edges[0].PlaceFrom,
      lastPlace = edges[edges.length - 1].PlaceTo;
    console.log(
      firstPlace.Id === lastPlace.Id,
      "firstPlace",
      firstPlace,
      "lastPlace",
      lastPlace
    ...
    this.setState({
      isComplete: firstPlace.Id === lastPlace.Id ? true : false,
    });
    edges.forEach(function (edge, i) {
      markerFrom = {
        id: edge.PlaceFrom.Id,
        lat: edge.PlaceFrom.Latitude,
        lng: edge.PlaceFrom.Longitude,
        text_mark: edge.PlaceFrom.Name,
      };
      markerTo = {
        id: edge.PlaceTo.Id,
        lat: edge.PlaceTo.Latitude,
        lng: edge.PlaceTo.Longitude,
        text_mark: edge.PlaceTo.Name,
      };
      if (markers === undefined) {
        markers = [markerFrom];
        markers.push(markerTo);
        markers2PolyView = [
          { id: markerFrom.id, lat: markerFrom.lat, lng: markerFrom.lng },
        ];
        markers2PolyView.push({
          id: markerTo.id,
          lat: markerTo.lat,
          lng: markerTo.lng,
        });
        ...
        if (
          markers.filter((marker) => marker.id === markerFrom.id).length === 0
        )
        ...
        >
          ...
          {!this.state.isComplete &&
            this.state.markers2PolyView[0].lat !== "" && (
              <Polyline
                path={this.state.markers2PolyView}
                strokeColor="#00FFFF"
              />
            )}
        </Map>
