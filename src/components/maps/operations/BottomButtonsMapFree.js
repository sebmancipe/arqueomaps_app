...
class BottomButtonsMapFree extends Component {
  ...
  setProjected = (e) => {
    this.props.BottomButtonsMapProps.isProjectable(
      this.state.isProjected ? true : false
    );
  ...
  setAngles = (e) => {
    this.props.BottomButtonsMapProps.setAngles(
      this.props.BottomButtonsMapProps.getAngle ? true : false
    );
  };
  ...
  submitProjection = (e) => {
    ...
    const lat1 = latSource.toRadians(),
      long1 = lngSource.toRadians();

    const sinlat2 =
      Math.sin(lat1) * Math.cos(angular_distance) +
      Math.cos(lat1) * Math.sin(angular_distance) * Math.cos(angle_radians);
    const lat2 = Math.asin(sinlat2);
    const y =
      Math.sin(angle_radians) * Math.sin(angular_distance) * Math.cos(lat1);
  ...
  render() {
    ...
    return (
      <ButtonGroup
        ...
        className="buttonGroupHorizontal"
      >
        <Button size="sm" variant="secondary" onClick={this.resetPolylines}>
          Reset
        </Button>
        <Button size="sm" variant="secondary" onClick={this.showPolylineStack}>
          Stack
        </Button>
        <DropdownButton id="dropdown-basic-button" title="Manually" size="sm">
          <Dropdown.Item onClick={(e) => this.showPolylineManually(e, false)}>
            {this.state.showManually ? "Deactivate" : "Activate"}
          </Dropdown.Item>
          <Dropdown.Item onClick={(e) => this.showPolylineManually(e, true)}>
            Reset
          </Dropdown.Item>
        </DropdownButton>
        <DropdownButton id="dropdown-basic-button" title="Many2One" size="sm">
          <Dropdown.Item onClick={(e) => this.showPolylineMany2One(e, false)}>
            {this.state.showMany2One ? "Deactivate" : "Activate"}
          </Dropdown.Item>
          <Dropdown.Item onClick={(e) => this.showPolylineMany2One(e, true)}>
            Reset
          </Dropdown.Item>
        </DropdownButton>
        <DropdownButton id="dropdown-basic-button" title="Angles" size="sm">
          <Dropdown.Item onClick={this.setAngles}>
            {this.props.BottomButtonsMapProps.getAngle
              ? "Deactivate"
              : "Activate"}
          </Dropdown.Item>
        </DropdownButton>
              ...
              <Form onChange={this.handleChange}>
                <Form.Group controlId="form">
                  <Row>
                    <Col>
                      <Form.Text size="sm">
                        Distancia desde el punto seleccionado
                      </Form.Text>
                      <Form.Control
                        size="sm"
                        type="number"
                ...
                <Row>
                  <Col>
                    <div>
                      Lugar seleccionado:{" "}
                      {
                        this.props.BottomButtonsMapProps.markerSelected
                          .text_mark
                      }
                    </div>
                  </Col>
                ...
                <Row>
                  <Col>
                    <Button
                      ...
                      type="button"
                      size="sm"
                      onClick={(e) => this.submitProjection(e)}
                    >
                      Add
                    </Button>
                  </Col>
                  <Col>
                    <Button
                      ...
                      type="button"
                      size="sm"
                      onClick={(e) => this.setProjected(e)}
                    >
                      {this.state.isProjected ? "Reset" : "Activate"}
                    </Button>
                  </Col>
                </Row>
              </Form>
